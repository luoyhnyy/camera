...
# 64bit
# try:
#     MVSDKdll = windll.LoadLibrary(os.path.abspath('.')+r"\MVSDKmd.dll")
# except:
#     pass
os.environ['path'] += ';%s'%(os.path.abspath('.'))   #添加dll依赖库目录到系统环境
# MVSDKdll = cdll.LoadLibrary('MVSDKmd.dll')
# MVSDKdll = OleDLL(os.path.abspath('.')+r"\MVSDKmd.dll")
# MVSDKdll = OleDLL(r"./MVSDKmd.dll")
if struct.calcsize("P") != 4:
    # os.add_dll_directory(os.path.abspath('.'))
    # MVSDKdll = CDLL("./MVSDKmd.dll", winmode=0)
    MVSDKdll = CDLL("./MVSDKmd.dll")
    # MVSDKdll = windll.LoadLibrary(r"./MVSDKmd.dll")
else:
    MVSDKdll = OleDLL(r"./MVSDKmd32.dll")
